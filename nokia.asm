...
drawBlock:
	...
	mov		#7, R12				; Put paddle at bottom of screen
...
endDrawLoop:
	...
	;-------------------------------------------------------------------------------
;	Name:		drawPaddleAsm
;	Inputs:		R12 column to draw paddle
;				R14 true if inverted colors
;	Outputs:	none
;	Purpose:	draw a paddle at row 8,8*col
;	Registers:	R5	column counter to draw all 8 pixel columns
;-------------------------------------------------------------------------------
drawPaddleAsm:
	push	R5
	...
	pop		R12					; Move R13 (column) to R12
	push	R12
	mov		#8, R12				; Put paddle at bottom of screen
	call	#setAddress			; move cursor to upper left corner of block

	mov		#1, R12				; data byte

	mov.w	#0x16, R5			; loop 8 pixel columns
	mov		#0xFF, r13			; All pixels black
loopBlackPaddle:
	call	#writeNokiaByte		; draw the pixels
	dec.w	R5
	jnz		loopBlackPaddle
